...
from trainers.t_ddpm import DDPMTrainer
...
def main():
    ...
    assert args["dataset"] is not None, "Specify a txt file with the image paths"
    unet_kwargs = read_model_config(args["unet_config"])

    dset_train = RGMasksDataset(
        args["data_root"], args["dataset"], img_size=32, mask_only=True
    )
    dset_test = RGMasksDataset(
        args["data_root"], args["dataset"], img_size=32, mask_only=True
    )
    ...
    # Setup trainer
    trainer = DDPMTrainer(args["lr"], default_kwargs, ddpm_kwargs, args["resume"])
